...
ADD https://download.elastic.co/beats/filebeat/filebeat-$FILEBEAT_VERSION-x86_64.tar.gz /tmp
...
RUN tar -C /usr/local/bin -xzvf /tmp/forego-stable-linux-amd64.tgz \
  ...
  && tar -C /tmp -xvzf /tmp/filebeat-$FILEBEAT_VERSION-x86_64.tar.gz \
  && mv /tmp/filebeat-$FILEBEAT_VERSION-x86_64/filebeat /usr/local/bin \
  && rm -r /tmp/filebeat-$FILEBEAT_VERSION-x86_64 /tmp/filebeat-$FILEBEAT_VERSION-x86_64.tar.gz \
